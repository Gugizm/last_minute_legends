    ...
    def process_new_movies(self):
        ...
        for row in new_movies_df.collect():
            movie_event = MovieCatalogEvent(
                ...
                title=row["Title"],
                genre=row["Genre"],
                list_price=row["ListPrice"]  # ✅ Now properly cleaned
            )
            self.kafka_publisher.send_event("movie_catalog_topic", movie_event.to_dict())
...
if __name__ == "__main__":
    ...
    kafka_publisher = KafkaEventPublisher(producer_conf)
